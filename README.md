...
| **コンポーネントファクトリ** | **`pkg/scraperfactory`** | **すべてのコア依存関係を構築・注入し、`scraperrunner.Runner` を初期化するファクトリ層。** |
| **実行管理** | **`pkg/scraperrunner`** | **フィード解析から並列スクレイピング実行までのオーケストレーションを担当。** |
...
| `--concurrency` | `-c` | 最大並列実行数。同時に処理する記事の数を制御します。`(Default: 6)` |
